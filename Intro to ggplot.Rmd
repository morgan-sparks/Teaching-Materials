...
setwd("~/Dropbox/PhD Work/TA Items/Ecological Statistics_Fall 2018/")
...
ggplot(iris_data, aes(x = Petal.Length)) +
  geom_histogram()
```
Perhaps we don't like our bin size, we can easily change it with the following code.  Binwidth here is based on your data and is up to you to choose.  Here we specifify bins for every 0.1 cm.
...
We then use the `emmeans` package to produce a table of these results.  We run the `emmeans()` by telling it what model (iris_reg) and what categorical variable (Species) we are interested in.  It will print a data table with the least squares means, as well as other summary information.
...
__Note: We recommend using this *general* script for producing figures for this course, as well as journals.  Please keep in mind the technique to remove color dispplayed a few examples prior, if color figures are not necessary.__
